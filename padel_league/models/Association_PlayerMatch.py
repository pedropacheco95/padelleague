...
class Association_PlayerMatch(db.Model ,model.Model):
    ...
    def get_create_form(self):
        ...
        # Info block
        fields = [
            get_field(name='player', label='Jogador', type='ManyToOne', required=True, related_model='Player'),
            get_field(name='match', label='Jogo', type='ManyToOne', required=True, related_model='Match'),
            get_field(name='team', label='Equipa', type='Select', required=True, options=['Home','Away']),
        ]
