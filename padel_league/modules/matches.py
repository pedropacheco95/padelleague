...
def by_matchweek(matchweek, division=None):
    ...
    if not division:
        ...
        matches = [
            match for match in division.matches if match.matchweek == int(matchweek)
        ]
    return render_template(
        "matches/matches.html", matches=matches, matchweek=int(matchweek)
    )
...
def for_edit(division_id=None):
    ...
    matches = [
        match for match in matches if match.date_hour <= tomorrow and (not match.played)
    ]
    return render_template(
        "matches/matches_for_edit.html",
        matches=matches,
        divisions=divisions,
        division=division,
    )
...
def edit(id):
    ...
    if request.method == "POST":
        eliminated_players = [
            player for player in request.form["players_eliminated"].split(";") if player
        ]
        if eliminated_players:
            ...
            for player in eliminated_players:
                association = Association_PlayerMatch.query.filter_by(
                    match_id=match.id, player_id=players[player].id
                ).first()
                association.delete()
        ...
        match.winner = (
            1
            if hometeam_games > awayteam_games
            else -1 if awayteam_games > hometeam_games else 0
        )
...
def create(division_id):
    ...
    if request.method == "POST":
        date_hour = datetime.datetime.strptime(
            request.form["date_hour"], "%Y-%m-%dT%H:%M"
        )
        hometeam_games_input = int(request.form["hometeam_games_input"])
        awayteam_games_input = int(request.form["awayteam_games_input"])
        home = [
            int(request.form["homeplayer0_id"]),
            int(request.form["homeplayer1_id"]),
        ]
        away = [
            int(request.form["awayplayer0_id"]),
            int(request.form["awayplayer1_id"]),
        ]
        winner = (
            1
            if hometeam_games_input > awayteam_games_input
            else -1 if awayteam_games_input > hometeam_games_input else 0
        )
        if len(list(set(home + away))) != 4:
            error = "Puseste o mesmo jogador duas vezes"
            flash(error)
            return render_template(
                "matches/create_match.html", division=division, players=players
            )
        players_in_match = {"home": home, "away": away}
        ...
        for player_id in players_in_match["home"]:
            association = Association_PlayerMatch(
                player_id=player_id, match_id=match.id, team="Home"
            )
            association.create()
        for player_id in players_in_match["away"]:
            association = Association_PlayerMatch(
                player_id=player_id, match_id=match.id, team="Away"
            )
            association.create()
