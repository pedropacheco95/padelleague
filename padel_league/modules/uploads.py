...
def upload_csv_to_db():
    ...
    for line in f:
        ...
        if columns[0] != "id":
            ...
            name = columns[1]
            beginning_datetime = (
                datetime.datetime.strptime(columns[2], "%Y-%m-%d %H:%M:%S")
                if columns[2]
                else None
            )
            rating = int(columns[3])
            end_date = (
                datetime.datetime.strptime(columns[4], "%Y-%m-%d")
                if columns[3]
                else None
            )
    ...
    for line in f:
        ...
        if columns[0] != "id":
            ...
            full_name = columns[2]
            birthday = (
                datetime.datetime.strptime(columns[3], "%Y-%m-%d")
                if columns[3]
                else None
            )
    ...
    for line in f:
        ...
        if columns[0] != "id":
            ...
            date_hour = (
                datetime.datetime.strptime(columns[3], "%Y-%m-%d %H:%M:%S")
                if columns[3]
                else None
            )
    ...
    for line in f:
        ...
        if columns[0] != "player_id":
            ...
            players_divisions = Association_PlayerDivision(
                player=player, division=division
            )
    ...
    for line in f:
        ...
        if columns[0] != "player_id":
            ...
            team = columns[2]
            players_matches = Association_PlayerMatch(
