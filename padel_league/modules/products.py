...
from padel_league.models import (
    Association_ProductProductAttribute,
...
def create():
    if request.method == "POST":
        ...
        for attribute in attributes:
            if attribute:
                association = Association_ProductProductAttribute(
                    product_id=product.id, product_attribute_id=int(attribute)
                )
                association.create()

        for value in attribute_values:
            if value:
                association = Association_ProductProductAttributeValue(
                    product_id=product.id, product_attribute_value_id=int(value)
                )
                association.create()
        ...
        for index in range(len(final_files)):
            ...
            if file.filename != "":
                ...
                image_name = "{image_name}_{product_id}_{index}.png".format(
                    image_name=image_name, product_id=product.id, index=index
                )
            ...
            for relation in product.product_attributes_relations:
                ...
                if value:
                    if attribute.user_input:
                        ...
                        values[attribute.name] = ProductAttributeValue.query.get(
                            int(value)
                        ).value
            values_string = [
                "{key}: {value}".format(key=key, value=values[key])
