...
from padel_league.models import Division, Edition, League, Player
...
def auto_create():
    ...
    if request.method == "POST":
        ...
        last_divisions = (
            Division.query.filter_by(edition_id=last_edition.id)
        ...
        for div in last_divisions:
            # associations = Association_PlayerDivision.query.filter_by(
            #     division_id=div.id
            # ).all()
        ...
        for i in range(num_divisions):
            ...
            if last_divisions and i < len(last_divisions):
                ...
                relegated_from_this = (
                    current_players[-2:] if i < len(last_divisions) - 1 else []
                )
                ...
                players = [
                    ...
                    if player not in relegated_from_this
                ]
                if promoted_to_above:
                    players = [
                        player for player in players if player not in promoted_to_above
                    ]
