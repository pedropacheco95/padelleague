...
def _add_fk_if_missing(insp, table, column, ref_table, ref_col="id", ondelete="SET NULL", name=None):
    if name is None:
...
def upgrade():
    ...
    if not _column_exists(insp, "players", "large_picture_id"):
        op.add_column("players", sa.Column("large_picture_id", sa.Integer(), nullable=True))
    ...
    # --- Add FKs if missing ---
    _add_fk_if_missing(insp, "players",  "picture_id",       "images", name="fk_players_picture_id_images")
    _add_fk_if_missing(insp, "players",  "large_picture_id", "images", name="fk_players_large_picture_id_images")
    _add_fk_if_missing(insp, "news",     "cover_image_id",   "images", name="fk_news_cover_image_id_images")
    _add_fk_if_missing(insp, "sponsors", "image_id",         "images", name="fk_sponsors_image_id_images")
    ...
    if not any(i['name'] == 'ix_players_picture_id' for i in insp.get_indexes('players')):
        op.create_index('ix_players_picture_id', 'players', ['picture_id'])
    if not any(i['name'] == 'ix_players_large_picture_id' for i in insp.get_indexes('players')):
    ...
    if not any(i['name'] == 'ix_sponsors_image_id' for i in insp.get_indexes('sponsors')):
